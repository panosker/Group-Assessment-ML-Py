...
per-topic affinity score from 4.2 (affinity_score_<topic>_scaled +
affinity_margin_scaled) - `4.4 compare_representations.py` already tested
Random Forest with these features; this notebook runs the full 3-model
comparison (Naive Bayes, KNN, Random Forest).

Run 4.1 and 4.2 first, so train_split_scored.csv/test_split_scored.csv
"""
...
Stacks the scaled affinity score columns from Section 4 onto the TF-IDF matrix,
